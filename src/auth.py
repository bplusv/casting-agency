...
class Auth:
    ...
    def get_token_auth_header():
        ...
        if parts[0].lower() != 'bearer':
            abort(401, description='Authorization header '
                                   'must start with bearer')
        elif len(parts) == 1:
            abort(401, description='Token not found')
        elif len(parts) > 2:
            abort(401, description='Authorization header must be '
                                   'bearer token')
        token = parts[1]
        return token
    ...
    def check_permissions(permission, payload):
        if 'permissions' not in payload:
            abort(403, description='Permissions payload missing')
    ...
    def verify_decode_jwt(token):
        ...
        if rsa_key:
            try:
                ...
                abort(401, description='JWT Token is expired')
            except jwt.JWTClaimsError:
                abort(401, description='JWT Incorrect claims, please check '
                                       'the audience and issuer')
            except Exception:
                abort(401, description='JWT Unable to parse token')
        abort(401, description='JWT unable to find appropriate key')
